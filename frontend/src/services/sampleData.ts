...
export function getClientSampleSummary(): TransportSummary {
  const summary = parseVl06oClient(
    RAW_SAMPLE_DATA,
    "3417089",
    "COMERCIAL DOLLINCO S.A.",
    "Semana 36",
    3
  );

  // Replicar las diferencias exactas de la muestra del usuario
  ...
  summary.total_cajas_preparadas = summary.items.reduce((a, b) => a + b.cantidad_preparada, 0);
  summary.skus_con_diferencia = summary.items.filter(i => i.tiene_diferencias === "Si").length;
